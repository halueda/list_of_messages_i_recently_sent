...
WScript.echo "Version= Time-stamp: <2019-04-19 17:31:03 ueda@SCR-00101636>"
...
'    Set oFolder = oSearchFolders("最近完了")
    Set oFolder = oSearchFolders("doneitem")
...
'    日付時刻 Subj12文字(半角24文字)以内 本文54文字
    ...
    strLine = Formatmmdd(mes.LastModificationTime)
'    WScript.echo strLine
    strLine = strLine & Formathhnn(mes.LastModificationTime)
'    WScript.echo strLine
    strLine = strLine & " 完 "
...
'    WScript.echo mes.Body
    shortBody = MyLeftB(mes.Body, 300)
...
'        .Pattern = ".*以下の承認依頼が事務処理支援.*サービス(InfoShare)にあります.*"
        .Pattern = "以下の承認依頼が事務処理支援サービス.*にあります"
        shortBody = .Replace(shortBody, "")
